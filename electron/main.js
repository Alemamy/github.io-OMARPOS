...
function normalizeStore(d){d=d&&typeof d==='object'?d:{};if(!Array.isArray(d.products))d.products=[];if(!Array.isArray(d.sales))d.sales=[];if(!Array.isArray(d.customers))d.customers=[];if(!Array.isArray(d.expenses))d.expenses=[];if(!d.settings||typeof d.settings!=='object')d.settings={...initial.settings};return d}
...
app.whenReady().then(()=>{ensureStore();
  ...
  ipcMain.handle('store:add-sale',(_,input)=>{const d=readStore();const sale=input&&typeof input==='object'?input:{};const items=Array.isArray(sale.items)?sale.items:[];if(!items.length)throw new Error('الفاتورة فارغة');const normalized=[];let subtotal=0;for(const item of items){const id=Number(item.id),qty=Number(item.qty),price=Number(item.price);if(!Number.isInteger(id)||!Number.isInteger(qty)||qty<=0||!Number.isFinite(price)||price<0)throw new Error('بيانات الصنف غير صحيحة');const p=d.products.find(x=>Number(x.id)===id);if(!p)throw new Error('الصنف غير موجود');if(Number(p.stock||0)<qty)throw new Error('المخزون غير كافٍ للصنف: '+p.name);normalized.push({id,name:String(p.name||item.name||''),qty,price});subtotal+=price*qty}const discount=Number(sale.discount||0);if(!Number.isFinite(discount)||discount<0||discount>subtotal)throw new Error('الخصم غير صحيح');const saved={items:normalized,subtotal:Number(subtotal.toFixed(2)),discount:Number(discount.toFixed(2)),total:Number((subtotal-discount).toFixed(2)),id:Date.now(),date:new Date().toISOString()};d.sales.push(saved);for(const item of normalized){const p=d.products.find(x=>Number(x.id)===item.id);p.stock=Math.max(0,Number(p.stock||0)-item.qty)}writeStore(d);return saved});
